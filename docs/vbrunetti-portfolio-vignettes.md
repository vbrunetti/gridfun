...
## NEXT SESSION — TODO: Connective Prose Pass on Cruise & Google

The Pearson chapter established a pattern: numbered craft vignettes + short "connective prose" sections that provide narrative glue, leadership/org context, and through-lines. Cruise and Google were captured before this pattern emerged and could benefit from the same treatment. Candidates below — pick what resonates, skip what doesn't.

**Cruise:**
- **Origin/setup** — How Victor landed the remote assistance terminal role, and what state the terminal was in when he arrived (mirrors Pearson's "no design system existed" opening)
- **The "context gain" throughline** — Vignettes 1, 2, 3, 5, 9 share a pattern: surfacing information the system already had but wasn't showing operators. Worth naming explicitly to give the chapter a spine
- **The operator-reality reframe** — Vignette 12's "operators are regular people, not specialists" insight may work better moved earlier as framing prose that recontextualizes the whole chapter
- **The shutdown** — Cruise ending affects Vignettes 8 (partially shipped) and 11 (never shipped). Could close the chapter with real weight — what that transition felt like, what happened to unfinished work

**Google:**
- **Cruise ↔ Google transition** — contrast in scale/context: one custom AV product vs. enterprise products across three teams
- **Bridging the three Google roles** — Store → Shopping Ads → Contact Center. Was there a throughline or reason for the moves?
- **Code Yellow context-setting** — a short lead-in establishing Google's internal power dynamics (ads team influence) before Vignette 17 lands
- **Google → Pearson transition** — why Victor left, especially relevant given "return to Google" is the throughline of the job search narrative
